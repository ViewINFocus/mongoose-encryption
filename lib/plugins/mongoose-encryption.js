...
var VERSION_BUF = Buffer.from(VERSION);
...
var mongooseEncryption = function(schema, options) {
    ...
    if (options.secret) {
        ...
        if (!options.encryptionKey || !options.signingKey) {
            ...
            encryptionKey = Buffer.from(options.encryptionKey, 'base64');
            if (encryptionKey.length !== 32) {
                throw new Error('options.encryptionKey must be a a 32 byte base64 string');
            }
            signingKey = Buffer.from(options.signingKey, 'base64');
    ...
    var computeAC = function(doc, fields, version, modelName) {
        ...
        var fullAuthenticationBuffer = Buffer.from(hmac.digest());
    ...
    if (options.middleware) { // defaults to true
        ...
        // mongoose 9 / kareem 3 removed the `next` callback from middleware: a hook must
        // return a promise for the save to wait on it. The callback-style methods below
        // stay as-is because `authenticate`/`decrypt` are public API.
        var callAsPromise = function(doc, method) {
        ...
        schema.pre('save', async function() {
            var that = this;
            var markAuthenticatedFields = function() {
            ...
            if (this.isNew || this.isSelected('_ct') ){
                await callAsPromise(that, 'encrypt');
                if ((that.isNew || allAuthenticationFieldsSelected(that)) && !isEmbeddedDocument(that)) {
                    markAuthenticatedFields();
                    await callAsPromise(that, 'sign');
                }
    ...
    schema.methods.sign = function(cb) {
        ...
        var authenticatedFieldsBuf = Buffer.from(JSON.stringify(authenticatedFields));
